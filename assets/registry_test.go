...
func TestComponentDefinition(t *testing.T) {
	def := ComponentDefinition{
		Name:    "test",
		Type:    reflect.TypeFor[mockComponent](),
		Size:    100,
		Factory: func(props map[string]any) (ecs.Component, error) { return &mockComponent{}, nil },
	}

	if def.Name != "test" {
		t.Errorf("Name = %q, want %q", def.Name, "test")
	}
	if def.Type != reflect.TypeFor[mockComponent]() {
		t.Error("Type mismatch")
	}
	if def.Size != 100 {
		t.Errorf("Size = %d, want 100", def.Size)
	}
	if def.Factory == nil {
		t.Error("Factory is nil")
	}
...
func TestComponentFactory(t *testing.T) {
	// Test that ComponentFactory type works
	var factory ComponentFactory = func(props map[string]any) (ecs.Component, error) {
		return &mockComponent{Name: "factory-test"}, nil
	}

	comp, err := factory(map[string]any{})
	if err != nil {
