...
func TestCamera_SetScreenSize(t *testing.T) {
	...
	if c.ScreenSize != (types.Vector2I{X: 800, Y: 600}) {
...
func TestCamera_WorldToScreenAndBack(t *testing.T) {
	...
	t.Run("world origin maps to screen center when camera is at origin", func(t *testing.T) {
		got := c.WorldToScreen(types.Vector2{})
		if got != (types.Vector2{X: 400, Y: 300}) {
	...
	t.Run("ScreenToWorld is the inverse of WorldToScreen", func(t *testing.T) {
		c.Position = types.Vector2{X: 120, Y: -45}
		c.Zoom = 2
		world := types.Vector2{X: 50, Y: -30}
...
func vecAlmostEqual(a, b types.Vector2) bool {
	const epsilon = 1e-9
...
func TestCamera_ViewportBounds(t *testing.T) {
	c := NewCamera()
	c.SetScreenSize(800, 600)
	c.Position = types.Vector2{X: 100, Y: 50}
	c.Zoom = 2

	got := c.ViewportBounds()
	want := types.Rect{
		Min: types.Vector2{X: 100 - 200, Y: 50 - 150},
		Max: types.Vector2{X: 100 + 200, Y: 50 + 150},
	}
...
func TestCamera_ClampPosition(t *testing.T) {
	t.Run("unbounded camera is never clamped", func(t *testing.T) {
		c := NewCamera()
		c.SetScreenSize(800, 600)
		c.Position = types.Vector2{X: 1e6, Y: -1e6}

		c.ClampPosition()

		if c.Position != (types.Vector2{X: 1e6, Y: -1e6}) {
	...
	t.Run("camera is pulled back inside explicit bounds", func(t *testing.T) {
		c := NewCamera()
		c.SetScreenSize(800, 600)
		c.Bounds = types.Rect{Min: types.Vector2{X: 0, Y: 0}, Max: types.Vector2{X: 1000, Y: 1000}}
		c.Position = types.Vector2{X: -500, Y: 2000}
...
func TestCamera_IsWorldRectVisible(t *testing.T) {
	...
	cases := []struct {
		...
		{"overlapping viewport", types.Rect{Min: types.Vector2{X: -10, Y: -10}, Max: types.Vector2{X: 10, Y: 10}}, true},
		{"far outside viewport", types.Rect{Min: types.Vector2{X: 10000, Y: 10000}, Max: types.Vector2{X: 10010, Y: 10010}}, false},
	}
