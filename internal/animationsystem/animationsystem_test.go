...
import (
	...
	pubanim "github.com/leonard-atorough/castrum/animation"
...
func TestSystem_Update_EmitsLoopEvent(t *testing.T) {
	...
	var emittedEvent pubanim.AnimationEvent
	var eventFired bool

	bus.On(func(_ events.EventMeta, e pubanim.AnimationEvent) {
		emittedEvent = e
		eventFired = true
	}, false)
	...
	if emittedEvent.Type != pubanim.EventClipLooped {
		t.Errorf("Event type = %d, want EventClipLooped (%d)", emittedEvent.Type, pubanim.EventClipLooped)
	}
}
...
func TestSystem_Update_StopsNonLoopingAnimation(t *testing.T) {
	...
	if emittedEvent.Type != pubanim.EventClipFinished {
		t.Errorf("Event type = %d, want EventClipFinished (%d)", emittedEvent.Type, pubanim.EventClipFinished)
	}
