...
func TestEntityIndex_ComponentIndex(t *testing.T) {
	...
	if got := idx.GetEntitiesWithComponent(atype); !idsMatchUnordered(got, []uint64{1, 2}) {
		t.Fatalf("unexpected A entities: %#v", got)
	}
	if got := idx.GetEntitiesWithComponent(btype); !idsMatchUnordered(got, []uint64{1}) {
		t.Fatalf("unexpected B entities: %#v", got)
	}
	if got := idx.GetEntitiesWithComponent(ctype); !idsMatchUnordered(got, []uint64{3}) {
		t.Fatalf("unexpected C entities: %#v", got)
	}

	if got := idx.GetEntitiesWithComponents(atype, btype); !idsMatchUnordered(got, []uint64{1}) {
	...
	if got := idx.GetEntitiesWithComponent(atype); !idsMatchUnordered(got, []uint64{2}) {
		t.Fatalf("after removal, A should only contain entity 2, got %#v", got)
	}

	idx.UpdateComponent(NewEntity(5, "x"), atype, true)
	if got := idx.GetEntitiesWithComponent(atype); !idsMatchUnordered(got, []uint64{2, 5}) {
...
func TestEntityIndex_TagsAndTemplates(t *testing.T) {
	...
	if got := idx.GetEntitiesWithTag("player"); !idsMatchUnordered(got, []uint64{1, 2}) {
		t.Fatalf("unexpected player entities: %#v", got)
	}
	if got := idx.GetEntitiesWithTag("npc"); !idsMatchUnordered(got, []uint64{3}) {
		t.Fatalf("unexpected npc entities: %#v", got)
	}

	idx.UpdateTag(NewEntity(1, "player"), "player", false)
	if got := idx.GetEntitiesWithTag("player"); !idsMatchUnordered(got, []uint64{2}) {
	...
	if got := idx.GetEntitiesWithTemplate("enemy"); !idsMatchUnordered(got, []uint64{11}) {
		t.Fatalf("expected enemy template to only contain entity 11, got %#v", got)
	}
	if got := idx.GetEntitiesWithTemplate("boss"); !idsMatchUnordered(got, []uint64{10}) {
...
func idsMatchUnordered(a, b []uint64) bool {
	if len(a) != len(b) {
		return false
	}
	if len(a) == 0 {
